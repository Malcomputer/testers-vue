    ...
    <div id='main-container'>
      <div id='top-bar'>
        <header>
          ...
          <div id='user-widget'>
            <button v-if='!activeUser' class='btn login-btn' type='button' onclick='location.replace(`/login?url=${location.pathname}`)'>
              <span>Log in</span>
            </button>
            <button v-if='activeUser' id='user-widget-btn' type='button'>
              <figure :title='this.userDisplayName' style='height: 30px; width: 30px; overflow: hidden;'>
                <img :src='this.userDisplayPhoto' :alt='this.userDisplayName'/>
              </figure>
...
export default {
  ...
  methods: {
    ...
    getGlobalData: () => console.log('Get global data'),
